<template>
	<div class="b__components b__loading_remote_data b__multi__select">
		<div class="b__multi__select__control">
			...
			<div class="input-control-wrap" v-if = "!isSingle || getSingleSelected() == null ">
				<input type="text" class="input-control" v-model="searchKeyword" :value = "searchKeyword" v-on:keydown.delete.stop="removeLastTag()"  v-on:keyup="change">
			</div>
		</div>
		<ul :class="listClasses">
			<li v-show = "searchList.length == 0" class="not-found">Not found</li>
			
			<li class="list-item"  v-for = "(item, index) in searchList" @click="toggleItem(item.id)">
