  ...
  mounted() {
    ...
    $.widget('ui.dialog', $.ui.dialog, {
      _allowInteraction: function (event) {
        return (
          !!$(event.target).closest('.mce-container').length ||
          this._super(event)
        )
      },
    }) //charm for tiny mce in modal :))
  ...
  methods: {
    callbackUpdateContent(callback) {
      ...
      if (_tiny) {
        ...
        if (this.contentTinyMCE != this.value) _tiny.setContent(this.value)
      } else callback()
    ...
    initTinyEditor() {
      ...
      var height =
        this.height == null || this.height == undefined ? '450' : this.height
      if (readonly == 1) var toolbar1 = false
      else
        var toolbar1 =
          'undo redo formatselect | bold italic strikethrough | link image | alignleft aligncenter alignright alignjustify | numlist bullist outdent indent pagebreak lineheightselect'
      tinymce.init(
        Object.assign(
          {},
          {
            ...
            theme: 'silver',
            plugins:
              'print preview searchreplace autolink directionality visualblocks visualchars image link template codesample table charmap hr pagebreak nonbreaking toc insertdatetime advlist lists textcolor wordcount imagetools contextmenu colorpicker textpattern help',
            toolbar1: toolbar1,
            ...
            templates:
              self.template == null || self.template == undefined
                ? []
                : self.template,
            content_css: [
            ...
            advlist_number_styles:
              'lower-alpha,lower-greek,lower-roman,upper-alpha,upper-roman,default',
            ...
            file_picker_callback: function (cb, value, meta) {
              ...
              $(
                '.mce-widget.mce-btn.mce-browsebutton.mce-abs-layout-item.mce-first.mce-btn-has-text button'
              ).trigger('click')
              return
            ...
            init_instance_callback: function (editor) {
              ...
              tinymce.activeEditor
                .getBody()
                .setAttribute('class', 'arc-custom-editor-body')
              tinymce.activeEditor
                .getBody()
                .setAttribute(
                  'style',
                  "overflow-y: scroll !important; font-family: 'Open Sans',sans-serif !important; font-size: 15px !important; padding: 1.5em;"
                )
              if (self.checkEdit != undefined) {
                tinymce.activeEditor
                  .getBody()
                  .setAttribute('contenteditable', false)
                tinymce.activeEditor.getBody().style.padding =
                  '10px 0px 0px 0px'
              }
