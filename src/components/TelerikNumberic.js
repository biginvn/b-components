...
export default {
  ...
  created() {
    if (
      parseInt(this.maxLength) != null &&
      parseInt(this.maxLength) != undefined &&
      parseInt(this.maxLength) <= this.inputMaxLength
    )
      this.inputMaxLength = parseInt(this.maxLength)
    if (parseInt(this.maxLength) <= this.inputMaxLength)
      console.log(
        'Exeption: Components Telerik Numerik props max-length must be <= 15 (interger format).'
      )
    this.initComponentData()
  },

  mounted() {
    if (this.value !== null) this.watchDataValue(this.value.toString())
    else this.watchDataValue('')
  },
  ...
  methods: {
    ...
    interruptInput(string) {
      if (string == '' || string == null || undefined) return ''
      string = parseInt(string)
      string = String(string)
      let strResult = ''
      let strSub = ''
      let loop = parseInt(string.length / 3)
      if (loop < 1) return string
      for (let i = 1; i <= loop; i++) {
        if (string.slice(0, string.length - 3) == '') {
          ...
          strSub =
            this.inputInterrupt + string.slice(string.length - 3, string.length)
          string = string.slice(0, string.length - 3)
        }
    ...
    checkValidateString(string) {
      ...
      if (string.split('.').length == 2) {
        ...
        if (
          string.split('.')[0] == null ||
          string.split('.')[0] == undefined ||
          string.split('.')[0] == ''
        )
          number = '0'
        else number = string.split('.')[0]
        if (
          string.split('.')[1] == null ||
          string.split('.')[1] == undefined ||
          string.split('.')[1] == ''
        )
          decimal = null
    ...
    affixInput(string) {
      ...
      let beginString =
        string.split('.')[0] == null || string.split('.')[0] == undefined
          ? ''
          : string.split('.')[0]
      let endString =
        string.split('.')[1] == null || string.split('.')[1] == undefined
          ? ''
          : string.split('.')[1]
      let decimalPoint =
        endString == '' || endString == null || endString == undefined
          ? ''
          : '.'
      if (string == '' || string == null || string == undefined)
      ...
      if (this.inputType == 'prefix') {
        this.inputValue =
          this.inputAffix +
          this.interruptInput(this.valueTemp) +
          decimalPoint +
          endString
      } else {
        this.inputValue =
