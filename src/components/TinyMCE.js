...
export default {
  ...
  computed: {
    isDisabled() {
      return this.disabled == 'disabled' ? 1 : 0
    },
    isReadOnly() {
      return this.checkEdit != undefined &&
        (!this.checkEdit || this.disabled == 'disabled')
        ? 1
        : 0
    },
  },
  ...
  methods: {
    ...
    eventListener() {
      $(document).on('focusin', function (e) {
        ...
        $('.tox-dropzone button input').attr({
      ...
      $.widget('ui.dialog', $.ui.dialog, {
        _allowInteraction: function (event) {
          return (
            !!$(event.target).closest('.tox-editor-container').length ||
            this._super(event)
          )
        },
      })
    ...
    init_instance_callback(editor) {
      ...
      editor
        ...
        .setAttribute(
          'style',
          "overflow-y: scroll !important; font-family: 'Open Sans',sans-serif !important; font-size: 15px !important; padding: 0.5em;"
        )
      ...
      editor.on('keyup', function (e) {
        if (e.which == 13) _this.$emit('event', e.which)

        if (this.getContent() != '') {
          if (_this.classLabel != 'active') _this.classLabel = 'active'
          return
        }
    ...
    file_picker_callback(callback, value, meta) {
      ...
      input.onchange = function () {
        ...
        var elLoading =
          '<div class="tox-dialog__busy-spinner page-loader arc-auto arc-loading-plugin"> ' +
          SvgLogo +
          ' </div>'
        if ($('.tox-dialog')) $('.tox-dialog').append(elLoading)
    ...
    updateFloatLabel(value) {
      var isEmpty =
        value == undefined || value == null || value == 0 || value == ''
          ? true
          : false
      if (!isEmpty) return (this.classLabel = 'active')
