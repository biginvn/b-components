    ...
    tags(val) {
      if (val.length == 0) {
        this.$nextTick(function () {
          $(this.$el)
            .find('.b__component_input_tag_wrapper label')
            .removeClass('active')
        })
      }
    },
  ...
  methods: {
    ...
    addNewTag(tag) {
      ...
      if (
        ...
        tag != null &&
        !tag.match(regex) &&
        ['zipcode', 'email'].includes(this.type)
      ) {
      } else {
    ...
    updateChange(value) {
      var isEmpty =
        value == undefined || value == '' || value.length == 0 ? true : false
      if (!isEmpty) {
    ...
    focusInputTag() {
      if (this.id != undefined) document.getElementById(this.id).focus()
    },
  },
}
