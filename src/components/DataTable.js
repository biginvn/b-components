...
export default {
  ...
  methods: {
    ...
    autoCalc() {
      $(this.bTable.column(0).footer()).html('Total: ')
      ...
      for (let i = 0; i < this.calcSum.length; i++) {
        if (this.calcSum[i].type == 'number') {
          ...
          // Update footer
          let currency
          this.calcSum[i].currency == null ||
          this.calcSum[i].currency == 'undefined'
            ? (currency = '')
            : (currency = this.calcSum[i].currency)
          ...
          // Update footer
          $(this.bTable.column(this.calcSum[i].col).footer()).html(total)
        }
      }
    },
    ...
    autoTfoot() {
      for (let i = 0; i < this.tableTfoot.length; i++) {
        $(this.bTable.column(i).footer()).html(this.tableTfoot[i].title)
      }
      ...
      $('#' + idTable + ' tbody').on('click', 'td.editable', function (e) {
        ...
        if (!$(this).hasClass('selected_cell')) {
          ...
          if ($(this).has('input').length) {
            ...
            $(this).append(
              '<input class="update_cell" type="text" value="' +
                data_cell +
                '" name="' +
                columns[colIndex].name +
                '"><button type="button" class="btn-update-cell btn-sm btn-primary">Update</button>'
            )
