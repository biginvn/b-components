  ...
  computed: {
    ...
    externalName() {
      if (
        ...
        if (selectItem.length > 0) return selectItem[0][this.itemText]
    ...
    externalLink() {
      if (
        this.isHyperLinkView &&
        (this.itemUrl !== undefined || this.itemUrl !== null) &&
        (this.value !== undefined || this.value !== null)
      ) {
        let value = this.value
        let selectItem = this.list.filter(
        ...
        if (selectItem.length > 0) return selectItem[0][this.itemUrl]
        return ''
      }
      return ''
    },
  ...
  methods: {
    ...
    isModelInList() {
      ...
      for (var i = 0; i < this.items.length; i++) {
        ...
        if (
          item.value != null &&
          item.value.toString() == this.value.toString()
        ) {
          found = true
          break
        }
